...
///
/// الدَّين الباقي في **سكربتات تشغيل وتقارير منقضية**، لا في محتوًى يراه طفل.
/// فالبوابة تُجمّد ما بقي ولا تسمح بنموّه: ملفٌ جديد أو عددٌ أكبر يُفشل الجولة.
/// وخفضُ العدد يتطلّب خفضَ الرقم هنا — فلا يبقى خطُّ الأساس أكبر من الواقع بصمت.
///
/// ## تصحيح (2026-09-23): «مزدوج وغير قابل للاستعادة» كان خطأً في المُرمِّز
///
/// كان هذا التعليق يقول إن التشويه **مزدوج**، ودليله «1097 محرف إبدال عند فكّ
/// `0088`». والرقم صحيح والاستنتاج خاطئ: 1097 هو بالضبط ما يُنتجه الفكّ
/// بـ**Latin-1**، وهو المُرمِّز الخطأ.
///
/// المقيس: التشويه **مفرد** بـWindows-1252 كما تُنفّذه ‎.NET — والدليل حاسم، إذ
/// يحمل الملف `U+0081` (×58) و`U+008F` (×3)، وهما موضعان **غير مُسنَدين** في
/// CP1252 يمرّرهما ذلك المُفكِّك وحده. والفكّ به:
///
///   * يُعيد الترميز فيُنتج الأصل **بايتًا ببايت** (فلا خسارة، وهو ما ينقض «مزدوج»)
///   * صفر `U+FFFD`
///   * هيكل SQL بمحارف ASCII **دون تغيير**
///   * و**150 من 150** عنوانًا طابقت `tools/tts/games/*.json` — المصدر الأصليّ —
///     بصفر اختلاف (114 مطابقة تامّة و36 بادئة مقتطعة لأن العنوان محدود الطول)
///
/// فالاستعادة تحقُّقٌ متقاطع لا دورةٌ عمياء، ولذلك خرج `0088` من هذه القائمة
/// بعد أن كان أكبر مدخل فيها.
///
