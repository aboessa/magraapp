...
test('أداة مقارنة السجل موجودة وتقرأ فقط', () => {
  ...
  /* ‏`--file` محليًّا و`--command` على البُعد — والسببان مقيسان لا مُفضَّلان.

     محليًّا `--file`: الاقتباس اليدوي عبر `cmd.exe` يُفسد كل استعلام فيه سطر
     جديد، وهو ما أضلّ تشخيص `DB-104` دورةً كاملة.

     وعلى البُعد `--command` **لازم**: قيس في 2026-09-23 أن
     `wrangler d1 execute --remote --file` يرفع الملف ويُشغّله **استيرادًا
     دفعيًّا**، فيُرجع ملخَّص الاستيراد (`Total queries executed`) لا صفوف
     `SELECT`. فكانت الأداة تقرأ صفًّا واحدًا باسم `undefined` وتستنتج أن 96
     ترحيلًا معلَّق على إنتاجٍ فيه 84 مُطبَّقًا — أي تُرشد إلى `migrate:remote`
     على قاعدةٍ مكتملة فيها 157 جدولًا و874 أصلًا.

     وما يحمي قيد الصدفة هو حرسٌ لا عُرف: استعلامُ البُعد يُرفَض إن حمل سطرًا
     جديدًا أو اقتباسًا مزدوجًا. */
  assert.match(tool, /'--file'/);
  assert.match(tool, /'--command'/);
  assert.match(
    tool,
    /\/\[\\n"\]\/\.test\(statement\)/,
    'استعلام البُعد يجب أن يُرفَض إن حمل سطرًا جديدًا أو اقتباسًا مزدوجًا',
  );

  /* واختيار الحِمل بالشكل لا بالموضع.

     `--remote` يطبع حِملين: ملخَّصًا ثم النتائج. فـ«أوّل قوس» و«أوّل JSON صالح»
     كلاهما يعطي الملخَّص — وقد أعطاه فعلًا في نسختين متعاقبتين من هذا الإصلاح. */
  assert.match(tool, /Array\.isArray\(item\?\.results\)/);
  assert.match(tool, /--self-test/, 'الأداة تُثبِت قدرة مُحلِّلها على التمييز');
