...
### المفتاح `r2_key` لا `id` — ولماذا، مقيسًا

نسخةٌ أولى من هذا الترحيل كانت `WHERE id = …` بمعرّفات `0088`، فأصابت **44 صفًّا من
150** على الإنتاج. والسبب أن نفس الملفات مُسجَّلة هناك **باصطلاحَي معرّف**: 44 بـ
`asset-games-*` و106 بـ`asset-vo-*` — **150 صفًّا و150 مفتاحًا مختلفًا**، أي لا
تكرار ولا اصطلاح واحد. فالثابت بين البيئات هو **الملف** لا المعرّف.

### تحفّظ على `0088` — وقع بشكلٍ **عكس** ما توقّعتُه

كان مكتوبًا هنا أن تطبيق `0088` على الإنتاج سيُنشئ 106 صفوف مكرّرة. والذي حدث
عكسه: قيود التفرُّد على `r2_key`/`expected_path` جعلت `INSERT OR IGNORE` **يتخطّى**
الـ150 بصمت، فلم يُنشأ صفٌّ واحد — ثم كسر `0089` مراجعَ `voice_manifest`. التفصيل
والإصلاح في قسم الإنتاج أدناه (`0096`).
...
تُصفِّر حجمًا صحيحًا ضبطه شيءٌ آخر.

**مُطبَّق على البيئتين في 2026-09-23، والمقيس بعده:**

| | محليًّا | الإنتاج |
|---|---:|---:|
| صفوف صوت الألعاب | 150 | **150** |
| مجموع البايتات | 27,710,280 | **27,710,280** |
| ما بقي بحجم صفر | 0 | **0** |
| أي أصلٍ له `r2_key` وحجمه صفر في القاعدة كلّها | — | **0** |

وأُعيد تشغيله محليًّا: 150 بيانًا نجحت والمجموع كما هو — فاللاعملية مُثبَتة لا
مُدَّعاة. و14 صفًّا صوتيًّا تبقى صفرًا **بحقّ**، وهي المبندلة بلا `r2_key`.
...
## الإنتاج صار مُطابقًا للمستودع — 2026-09-23، وما كلّفه ذلك

كان الإنتاج يحمل **84 ترحيلًا مُطبَّقًا و15 معلَّقًا**. صار **صفر معلَّق** (99 ملفًّا
مُطبَّقة + 3 أيتام). ونسخة ما قبل الدفعة:
`.backups/majarra-db-prod-pre-pending13-20260923.sql` (12,801,792 بايت).

ولم تكن الدفعة نظيفة — عطلان حقيقيان ظهرا، وثالثٌ **أحدثتُه**:

### `0084` فشل لأن العطل الذي يمنعه كان قد وقع بالفعل

`UNIQUE constraint failed: ops_alerts.fingerprint`. والمقيس: **11,640 صفَّ تنبيه،
منها 11,574 حيًّا، وبصمتان فقط** — `dlq:pending` (5,820) و`queue:stale` (5,754)،
كلتاهما `high` **ومفتوحة منذ 2026-09-03**.

أي أن شاشة العمليات عرضت أحد عشر ألف تنبيه نشط عن **مشكلتين** عشرين يومًا. وذلك
بالحرف ما يصفه تعليق `0084` («٢٨٨ صفًّا عن مشكلة واحدة ضجيجٌ يُدرِّب من يقرأه على
تجاهله») — فالترحيل الذي يُصلح الضجيج كان محجوبًا بالضجيج.

و`0095_ops_alert_dedupe.sql` يطوي المكرّرات مُبقيًا **الأقدم** لكل بصمة (فـ
`started_at` يبقى صادقًا كما يشترط `0084`)، ويحلّها ولا يحذفها. المقيس بعده:
**11,574 → 2**، والناجيان بتاريخهما الأصلي `dlq:pending@2026-09-03 20:43:15` و
`queue:stale@2026-09-03 22:00:59`.

**وقراءة التنبيهين:** `queue_health` يقول إن `family_events` آخر نجاح له
2026-09-03 20:59:17 وحالته `degraded`، و51 حدثًا في `failed_family_events`. والإنتاج
فيه **صفر أسرة**: لا أحداث تُنتَج، فالمستهلك لا يعمل، فـ`last_success_at` لا
يتقدّم. أي أن المحرّك يُبلّغ **الخمول** على أنه **تدهور** — عطلٌ في دلالة التنبيه لا
في الطابور، وسيستمرّ ما دامت المنصّة قبل الإطلاق. إصلاحه قرارٌ في معنى `degraded`
بلا حِمل، ولم يُتَّخذ.

### ⚠ `0089` كسر صوت الألعاب على الإنتاج، و`0096` أصلحه

التحفّظ المكتوب سابقًا هنا توقّع أن `0088` سيُنشئ 106 صفوف **مكرّرة**. والذي حدث
**عكسه**: `content_assets` فيه قيود تفرُّد على `r2_key`/`expected_path` (ثلاثة
`sqlite_autoindex`)، فإدراجات `0088` بـ`INSERT OR IGNORE` **تُخطِّيت بصمت** — بقيت
150 صفًّا و150 مفتاحًا، 44 بمعرّفات `asset-games-*` و106 بـ`asset-vo-*`.

ثم كتب `0089` خرائط `voice_manifest` تشير إلى معرّفات `asset-games-*` **لم تُنشأ**.
والمقيس: 236 مرجعًا في `voice_manifest`، منها **134 مكسورًا** — أي ألعابٌ تُشغَّل
بلا سرد صوتي.

و`OR IGNORE` هو ما جعله صامتًا: منع الفشل ومنع معه الخبر. **والدرس:** `OR IGNORE`
على المفتاح الأوّلي بينما التفرُّد الحقيقي على عمودٍ آخر يُنتج تخطّيًا لا يُبلَّغ عنه.

`0096_voice_manifest_actual_ids.sql` يُعيد توجيه المراجع الـ106 إلى المعرّفات التي
يحملها الإنتاج فعلًا، **مطابقةً بـ`r2_key`** — فالملف هو الثابت والمعرّف اصطلاح،
وهو نفس الدرس الذي فرض على `0094` مفتاحه. والحرس `instr()` لا `LIKE`: نسخةٌ أولى
رفضها D1 بـ`LIKE or GLOB pattern too complex`.
