...
### ⚠ تحفّظ على `0088` قبل تطبيق المعلَّق على الإنتاج

`0088` يُدرج الـ150 كلّها بمعرّفات `asset-games-*`. والإنتاج يحمل **106 منها
بمعرّفات `asset-vo-*` بالفعل**، وإدراجاته `INSERT OR IGNORE` **على المعرّف** لا على
`r2_key` — فالتطبيق هناك يُنشئ **106 صفوف جديدة** تشير إلى نفس الملفات، أي تكرارًا
حقيقيًّا في كتالوج الأصول.

وهذا يجب أن يُحسم **قبل** تطبيق الترحيلات المعلَّقة على الإنتاج: إمّا قيدُ تفرُّدٍ
على `r2_key`، أو تحويل `0088` إلى `WHERE NOT EXISTS (… r2_key …)`، أو توحيد
الاصطلاح بترحيل يسبقه. ولم أختر بينها — الخيار يمسّ 106 صفوف في الإنتاج.
