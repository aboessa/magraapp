...
booksRoute.post('/:id/audio-sessions', async (c) => {
  ...
  // `API-201` / `DECIDE-108` — **القرار اتُّخذ (2026-09-23، المالك):** حدود الوقت
  // ووقت النوم **لا تُفرَض إلا إذا فعّلها ولي الأمر**، وحين يفعّلها فهي تسري على
  // السرد الصوتي كما تسري على الفيديو.
  //
  // ## ما يجعل هذا الربط مطابقًا للقرار، لا تجاوزًا له
  //
  // لا علَم تفعيلٍ منفصل، ولا حاجة إليه: الضبط **هو** التفعيل. `parentalControls.ts`
  // يُرجع `bedtimeActive: false` ما لم يضبط ولي الأمر نافذةً كاملة (`bedtime_start`
  // و`bedtime_end` معًا وبصيغة صحيحة)، و`dailyMinutes: null` ما لم يضبط رقمًا
  // موجبًا. فأسرةٌ لم تلمس الإعدادات تمرّ من هنا بلا فحصٍ فعليّ — وهذا مُثبَت
  // باختبار في `test/parentalControls.test.mjs` لا مستنتَجًا من قراءة الكود.
  //
  // وسؤال `DECIDE-108` الأوّل — «هل حكاية قبل النوم ممنوعة في وقت النوم؟» — جوابه
  // بهذا القرار: **نعم، إن فعّل ولي الأمر وقت النوم**. فمن يريد حكاية داخل تلك
  // النافذة لا يضبطها، أو يضبط بدايتها بعد موعد الحكاية. السلطة لولي الأمر لا
  // لتخمينٍ في الخادم.
  //
  // ## وما زال خارج هذا الربط
  //
  // الاستماع **لا يستهلم** الرصيد اليومي، إنما يُفحَص عليه: `creditWatchTime` يحتاج
  // فارقًا بين نبضتين، والسرد بلا نبضة — ومنحُ توكنٍ ليس دليلًا على أن أحدًا سمع.
  // فالاحتساب يحتاج إشارة تقدُّم استماع من العميل، وهو عملٌ في الطرفين لا سطر هنا.
  //
  // والبوابة **لا تُنشئ عقد تشغيل**: العقد للتزامن، والتزامن مقصورٌ على الفيديو كما
  // تُبرّر الفقرة أعلاه — فعدُّ حكايةٍ عليه كان سيمنع أبًا يقرأ لطفل بينما يشاهد آخر.
