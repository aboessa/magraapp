...
export function SeasonsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try { const [seasons, seriesResponse] = await Promise.all([api.seasons(filter || undefined), api.series({ status: 'all', limit: 100 })]); setItems(seasons.data); setSeries(seriesResponse.data.filter((item) => item.status !== 'archived')) }
