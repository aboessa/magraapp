export type IconName =
  ...
  | 'grid' | 'bars' | 'copy' | 'layers'
  | 'star' | 'palette' | 'video' | 'chevron-left' | 'info' | 'chat'
  | 'shield' | 'users' | 'tag' | 'packages' | 'pen' | 'lock' | 'web-pages' | 'alert-triangle' | 'review' | 'download'
  | 'dollar' | 'trending-up' | 'file-text' | 'sliders'

export function Icon({ name, size = 20, style }: { name: IconName; size?: number; style?: React.CSSProperties }) {
  const common = { width: size, height: size, viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', strokeWidth: 1.8, strokeLinecap: 'round' as const, strokeLinejoin: 'round' as const, 'aria-hidden': true, style }

  switch (name) {
    case 'grid': return <svg {...common}><rect x="3" y="3" width="7" height="7" rx="1.5"/><rect x="14" y="3" width="7" height="7" rx="1.5"/><rect x="14" y="14" width="7" height="7" rx="1.5"/><rect x="3" y="14" width="7" height="7" rx="1.5"/></svg>
    case 'bars': return <svg {...common}><path d="M4 6h16M4 12h16M4 18h16"/></svg>
    case 'copy': return <svg {...common}><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>
    case 'layers': return <svg {...common}><path d="m12.83 2.18a2 2 0 0 0-1.66 0L2.6 6.08a1 1 0 0 0 0 1.83l8.58 3.91a2 2 0 0 0 1.66 0l8.58-3.9a1 1 0 0 0 0-1.83Z"/><path d="m22 12.5-8.58 3.91a2 2 0 0 1-1.66 0L2 12.5"/><path d="m22 17.5-8.58 3.91a2 2 0 0 1-1.66 0L2 17.5"/></svg>
    ...
    case 'chat': return <svg {...common}><path d="M7.9 20A9 9 0 1 0 4 16.1L2 22Z"/></svg>
    ...
    case 'tag': return <svg {...common}><path d="M12 2H2v10l9.29 9.29c.94.94 2.48.94 3.42 0l6.58-6.58c.94-.94.94-2.48 0-3.42L12 2Z"/><circle cx="7" cy="7" r=".5" fill="currentColor"/></svg>
    case 'packages': return <svg {...common}><path d="m7.5 4.27 9 5.15M21 8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16Z"/><path d="m3.3 7 8.7 5 8.7-5M12 22V12"/></svg>
    case 'pen': return <svg {...common}><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/></svg>
    case 'lock': return <svg {...common}><rect width="18" height="11" x="3" y="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
    case 'web-pages': return <svg {...common}><rect x="2" y="4" width="20" height="15" rx="2"/><path d="M2 9h20M6 6.5h.01M9 6.5h.01"/></svg>
    case 'alert-triangle': return <svg {...common}><path d="M12 3 2 20h20Z"/><path d="M12 9v5M12 17h.01"/></svg>
    case 'review': return <svg {...common}><rect x="4" y="4" width="16" height="14" rx="2"/><path d="M8 9h8M8 13h5M9 18l3 3 3-3"/></svg>
