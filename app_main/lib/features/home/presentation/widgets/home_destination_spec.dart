...
/// icon pair (outlined / filled-selected) and the screen it opens.
///
/// Phone, tablet and television shells all derive their navigation rows from
/// [buildHomeDestinationSpecs] instead of writing their own labels and icons,
/// so the three can no longer drift apart the way they had — the phone bottom
/// bar called index 2 "مكتبتي" while the tablet and television rails called
/// the exact same body "بحث" (spec app-foundation-family-journey,
/// Requirement 3).
class HomeDestinationSpec {
  const HomeDestinationSpec({
    required this.label,
    required this.icon,
...
/// Builds the four canonical destinations in index order: home, explore,
/// library, profile.
///
/// This is deliberately free of any dependency on `WidgetRef`/Riverpod beyond
/// what the destination bodies themselves already need, so it stays usable
/// from any shell without forcing a `ConsumerWidget` on every caller.
/// Per-surface dynamic overrides — such as the phone bottom bar replacing the
/// profile label with the active child's name — are layered on top by the
/// caller (see `majarra_bottom_navigation.dart`) instead of leaking that
/// concern into this shared spec.
List<HomeDestinationSpec> buildHomeDestinationSpecs({
...
/// Explains, once, why account management is unavailable in the guest session
/// and offers the only action that can change it.
class _GuestAccountNotice extends StatelessWidget {
  const _GuestAccountNotice();
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(
      ...
      border: Border.all(color: AppColors.starGold.withValues(alpha: 0.34)),
    ),
    child: Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        Row(
          ...
          children: [
            ...
            Expanded(
              child: Text(
                'أنت في تجربة الضيف. بيانات الحساب والأجهزة والإعدادات ومنطقة '
                'ولي الأمر تحتاج حساب أسرة.',
                style: TextStyle(
...
/// Guest taps on an account-only action get a stated reason and a way forward
/// rather than a silent redirect back to the same screen.
void _promptSignIn(BuildContext context, String feature) {
  ...
  messenger?.showSnackBar(
    SnackBar(
      ...
      content: Text(
        '«$feature» يحتاج حساب أسرة. سجّل الدخول للمتابعة.',
...
class _ProfileSettingTile extends StatelessWidget {
  ...
  /// Short state note, e.g. that the row needs a real account. Announced with
  /// the label so the requirement is not colour- or layout-only.
  final String? trailingNote;

  @override
  Widget build(BuildContext context) => FocusableScale(
    onPressed: onTap,
    semanticLabel: trailingNote == null ? label : '$label، $trailingNote',
    borderRadius: BorderRadius.circular(14),
    child: Container(
          ...
          if (trailingNote != null) ...[
            Flexible(
              child: Text(
                ...
                style: TextStyle(
                  color: AppColors.starGold.withValues(alpha: 0.86),
                  fontSize: 10.5,
                  fontWeight: FontWeight.w700,
                ),
              ),
